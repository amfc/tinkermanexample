...
DAP_DragAndDrop.prototype.onMouseMove = function(event)
{
    var pos = DOM_GetPositionFromEvent(event);
    this.element.style.left = pos.x - this.drag_x + 'px';
    this.element.style.top = pos.y - this.drag_y + 'px';
}
...
DAP_DragAndDrop.prototype.grabDrag = function(event)
{
    DAP_Dragging_obj = this;
    this.element.className = VAR_AddWord(this.element.className, this.class_name + '-dragging');
    var body = document.getElementsByTagName('body')[0];
    body.className = VAR_AddWord(body.className, 'dragging');
    DOM_AddObjEventListener(this, body, 'mousemove', this.onMouseMove);
    this.old_position = this.element.style.position;
    this.old_left = this.element.style.left;
    this.old_top = this.element.style.top;
    
    var pos_event = DOM_GetPositionFromEvent(event);
    var pos = DOM_GetPosition(this.element);
    
    this.drag_x = pos_event.x - pos.x;
    this.drag_y = pos_event.y - pos.y;
...
DAP_DragAndDrop.prototype.releaseDrag = function(event)
{
    ...
    DOM_RemoveObjEventListener(this, body, 'mousemove', this.onMouseMove);
    this.element.style.position = this.old_position;
    this.element.style.left = this.old_left;
    this.element.style.top = this.old_top;
}
